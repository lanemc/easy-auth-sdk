...
function Connection(options, queues = {}, { onopen = noop$1, onend = noop$1, onclose = noop$1 } = {}) {
  ...
  function reconnect() {
    setTimeout(connect, closedDate ? closedDate + delay - perf_hooks.performance.now() : 0);
  }
  ...
  async function closed(hadError) {
    ...
    closedDate = perf_hooks.performance.now();
...
const AccountSchema = zod.z.object({
    ...
    providerAccountId: zod.z.string(),
...
// Auth configuration
const AuthConfigSchema = zod.z.object({
    database: zod.z.object({
        type: zod.z.literal('postgres'),
        url: zod.z.string().url()
    }),
    session: zod.z.object({
        strategy: zod.z.enum(['database', 'jwt']).default('database'),
        maxAge: zod.z.number().default(30 * 24 * 60 * 60), // 30 days
        secret: zod.z.string().min(32)
    }),
    providers: zod.z.object({
        emailPassword: zod.z.object({
            enabled: zod.z.boolean().default(false)
        }).optional(),
        google: zod.z.object({
            clientId: zod.z.string(),
            clientSecret: zod.z.string(),
            scope: zod.z.string().default('openid email profile')
        }).optional(),
        github: zod.z.object({
            clientId: zod.z.string(),
            clientSecret: zod.z.string(),
            scope: zod.z.string().default('user:email')
        }).optional()
    }),
    callbacks: zod.z.object({
        onSignUp: zod.z.function().args(zod.z.object({ user: UserSchema })).returns(zod.z.void()).optional(),
        onSignIn: zod.z.function().args(zod.z.object({ user: UserSchema, account: AccountSchema.optional() })).returns(zod.z.void()).optional(),
        onSignOut: zod.z.function().args(zod.z.object({ user: UserSchema })).returns(zod.z.void()).optional()
    }).optional()
});
// Error types
    ...
    async cleanupExpiredSessions() {
        try {
            const result = await this.db
                ...
                .where(drizzleOrm.sql `expires_at < NOW()`)
...
function generateId(prefix) {
    const id = nanoid.nanoid(21); // 21 characters for good uniqueness
...
function generateSessionToken() {
    return nanoid.nanoid(32); // Longer token for sessions
}
function generateVerificationToken() {
    return nanoid.nanoid(32);
}
// Password utilities
    ...
    async signUp(email, password, name) {
        try {
            ...
            const existingUser = await this.db.db
                .select()
                .from(schema.users)
                .where(drizzleOrm.eq(schema.users.email, email.toLowerCase()))
                .limit(1);
    ...
    async updatePassword(userId, oldPassword, newPassword) {
        try {
            ...
            const [user] = await this.db.db
                .select()
                .from(schema.users)
                .where(drizzleOrm.eq(schema.users.id, userId))
                .limit(1);
    ...
    async resetPassword(email, newPassword, token) {
        try {
            ...
            const [verificationToken] = await this.db.db
                ...
                .where(drizzleOrm.eq(schema.verificationTokens.token, token))
                .limit(1);
            ...
            await this.db.db
                .delete(schema.verificationTokens)
                .where(drizzleOrm.eq(schema.verificationTokens.id, verificationToken.id));
            return true;
        }
        catch (error) {
            if (error instanceof AuthError) {
...
class OAuthManager {
    ...
    async findOrCreateUser(profile, providerId, tokens) {
        try {
            ...
            const [existingAccount] = await this.db.db
                ...
                .innerJoin(schema.users, drizzleOrm.eq(schema.accounts.userId, schema.users.id))
                .where(drizzleOrm.and(drizzleOrm.eq(schema.accounts.provider, providerId), drizzleOrm.eq(schema.accounts.providerAccountId, profile.id)))
                .limit(1);
            if (existingAccount) {
                // Update tokens
            ...
            if (profile.email) {
                const [existingUser] = await this.db.db
                    .select()
                    .from(schema.users)
                    .where(drizzleOrm.eq(schema.users.email, profile.email.toLowerCase()))
                    .limit(1);
                if (existingUser) {
                    user = existingUser;
                }
    ...
    async updateAccountTokens(accountId, tokens) {
        await this.db.db
            ...
            .where(drizzleOrm.eq(schema.accounts.id, accountId));
    }
...
class SessionManager {
    ...
    async getDatabaseSession(sessionToken) {
        const [result] = await this.db.db
            ...
            .innerJoin(schema.users, drizzleOrm.eq(schema.sessions.userId, schema.users.id))
            .where(drizzleOrm.and(drizzleOrm.eq(schema.sessions.sessionToken, sessionToken), drizzleOrm.gt(schema.sessions.expiresAt, new Date())))
            .limit(1);
        if (!result) {
            return null;
        }
    ...
    async updateSession(sessionToken) {
        ...
        try {
            ...
            const [updatedSession] = await this.db.db
                ...
                .set({ updatedAt: now })
                .where(drizzleOrm.and(drizzleOrm.eq(schema.sessions.sessionToken, sessionToken), drizzleOrm.gt(schema.sessions.expiresAt, now)))
                .returning();
            if (!updatedSession) {
                return null;
            }
    ...
    async deleteSession(sessionToken) {
        ...
        try {
            const result = await this.db.db
                .delete(schema.sessions)
                .where(drizzleOrm.eq(schema.sessions.sessionToken, sessionToken))
                .returning({ id: schema.sessions.id });
    ...
    async deleteAllUserSessions(userId) {
        ...
        try {
            const result = await this.db.db
                .delete(schema.sessions)
                .where(drizzleOrm.eq(schema.sessions.userId, userId))
                .returning({ id: schema.sessions.id });
            return result.length;
        }
        catch (error) {
...
class NextAuthAdapter {
    ...
    async handleSignOut(request) {
        ...
        try {
            ...
            if (sessionToken) {
                ...
                const response = server.NextResponse.json({ success: result.success });
    ...
    async handleGetSession(request) {
        ...
        try {
            const sessionToken = this.getSessionTokenFromRequest(request);
            if (!sessionToken) {
                return server.NextResponse.json({ user: null, session: null });
            }
    ...
    async handleOAuthRedirect(request, provider) {
        try {
            ...
            const response = server.NextResponse.redirect(authUrl);
            ...
            return server.NextResponse.redirect(new URL(errorUrl, request.url));
        }
    }
    async handleOAuthCallback(request, provider) {
        try {
            ...
            if (result.success && result.user && result.sessionCookie) {
                ...
                const response = server.NextResponse.redirect(new URL(redirectUrl, request.url));
                response.headers.set('Set-Cookie', result.sessionCookie);
                // Clear state cookie
    ...
    async handleGetProviders() {
        try {
            ...
            return server.NextResponse.json({ providers });
        }
        catch (error) {
            return server.NextResponse.json({ providers: [] });
        }
