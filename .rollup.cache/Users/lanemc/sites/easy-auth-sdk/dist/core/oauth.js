...
import { AuthError, ValidationError, DatabaseError } from '../types';
import { generateUserId, generateAccountId, buildURL } from '../utils';
    ...
    async findOrCreateUser(profile, providerId, tokens) {
        try {
            ...
            const [existingAccount] = await this.db.db
                ...
                .where(and(eq(schema.accounts.provider, providerId), eq(schema.accounts.providerAccountId, profile.id)))
                .limit(1);
            if (existingAccount) {
                // Update tokens
